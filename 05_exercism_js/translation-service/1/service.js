...
export class TranslationService {
 ...
 request(text) {
    ...
    return new Promise((resolve, reject) => {
      this.api.request(text, (error) => {
        if (error) {
          if (attemptsLeft > 1) {
            // Retry
            makeRequest(attemptsLeft - 1).then(resolve).catch(reject);
          } else {
            // Out of retries, reject with the error
            reject(error);
          }
        } else {
          resolve();
  ...
  return this.api.fetch(text)
    ...
    .catch(error => {
      // If it's a QualityThresholdNotMet error, just forward it
      if (error instanceof QualityThresholdNotMet) {
        throw error;
      }
      
      // Otherwise, translation is not available - request it
      return this.request(text)
        .then(() => this.api.fetch(text))
        .then(result => {
          // Check quality of the newly fetched translation
          if (result.quality < minimumQuality) {
            throw new QualityThresholdNotMet(text);
          }
          return result.translation;
